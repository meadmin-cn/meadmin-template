<template>
  <div class="footer">
    ...
    >
      <div class="list" ref="listRef">
        <div
          ...
          @click="push(tag)"
          @contextmenu.prevent="
            setContextmenu($event.currentTarget as any, tag)
          "
        >
          {{ $t(tag.meta.title!) }}
        </div>
    ...
    <div class="right">
      <div
        class="icon pointer"
        :class="{ 'is-disabled': scrollLeft >= max }"
        @click="go()"
      >
      ...
      <div
        class="icon pointer"
        v-if="themeConfig.tagBarRefresh"
        @click="reload"
      >
...
<script setup lang="ts" name="tagBar">
  ...
  onMounted(() => {
    max.value =
      listRef.value!.offsetWidth - scrollbarRef.value?.$el.clientWidth;
    mitter.on(
      event.resize,
      () => {
        max.value =
